...
\begin{itemize}
    ...
    \item The type of \gsl{DER} device connected to each bus. If the \gls{DER} device is directly connected to the medium voltage grid, the active power and reactive power are available for each time step. 
    %\emph{otherwise? only the injection at the 'higher' bus is known?}
\end{itemize}
