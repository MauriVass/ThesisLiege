% for glossary entry
% @entry{bird,
%     name={bird},
%     description = {feathered animal},
%     see={[see also]{duck,goose}}
% }

% if this bib file does not work, try using \input{file.tex}
% where all the \newabbreviation commands have been inserted
% containing all the definitions

% Gls to capitalize first letter
% GLS for full uppercase
% for abbreviations also
% glsxtrshort for abbreviation
% similar for long, full, and capital configurations, add pl at the end for plurals
% glsentryshort, long, plural (referred to shorts) must be used when in section titles
% glslink to allow the link but use a different text (as for href)
% glspl use plural form (add s to the end of the abbreviation)


% if you want to use also description for the abbreviations/acronyms, you should use bib2gls and define all the entries in a bib file, which is incompatible with Overleaf

%Legend:
%\newacronym {how you call it \gls{name}}{how it appears}{description}

\newacronym {ANN}{ANN}{Artificial neural network}
\newacronym {ANM}{ANM}{Active network management}
% \newacronym {ANSI}{ANSI}{American National Standards Institute}
\newacronym {API}{API}{Application programming interface}

% \newacronym {BBM}{BBM}{Bus-branch model}
\newacronym {BDEW}{BDEW}{German Association of Energy and Water Industries}

\newacronym {CNN}{CNN}{Convolution neural network}

\newacronym {DDPG}{DDPG}{Deep deterministic policy gradient}
\newacronym {DER}{DER}{Distributed energy resources}
\newacronym {DES}{DES}{Distributed energy storage}
\newacronym {DNN}{DNN}{Deep neural network}
\newacronym {DPG}{DPG}{Deterministic policy gradient}
\newacronym {DSO}{DSO}{Distribution system operator}

\newacronym {FC}{FC}{Fully connected}
\newacronym {FN}{FN}{False negative}
\newacronym {FP}{FP}{False positive}

% \newacronym {IEC}{IEC}{International Electrotechnical Commission}
\newacronym{iid}{i.i.d.}{Independently and identically distributed}

% \newacronym {FACTS}{FACTS}{Flexible alternating current transmission system}

% \newacronym {GARPUR}{GARPUR}{Generally accepted reliability principle with uncertainty modelling and through probabilistic risk assessment}
% \newacronym {GAN}{GAN}{Generative adversial network}
\newacronym {GHG}{GHG}{Greenhouse gas}

\newacronym {LSTM}{LSTM}{Long short-term memory}

\newacronym {MAE}{MAE}{Mean absolute error}
\newacronym {MDP}{MDP}{Markov Decision Process }
\newacronym {MLP}{MLP}{Multi layer perceptron}

% \newacronym {OPF}{OPF}{Optimal power flow}

\newacronym {PF}{PF}{Power flow}
% \newacronym {PPO}{PPO}{Proximal policy optimization}
\newacronym {pu}{p.u.}{Per unit}
\newacronym {PV}{PV}{Photovoltaic}

\newacronym {Relu}{ReLU}{Rectified linear unit}
\newacronym {RES}{RES}{Renewable energy sources}
\newacronym {RL}{RL}{Reinforcement learning}
\newacronym {RNN}{RNN}{Recurrent neural network}
% \newacronym {RNNGBRT}{RNN-GBRT}{Recurrent neural network gradient boosting regression tree}
\newacronym {rms}{r.m.s.}{Root mean square}

% \newacronym {SAC}{SAC}{Soft actor critic}
\newacronym {SLP}{SLP}{Standard load profile}
\newacronym {SMOTE}{SMOTE}{Synthetic minority oversampling technique}

\newacronym {Tanh}{Tanh}{Hyperbolic tangent function}
\newacronym {TN}{TN}{True negative}
\newacronym {TP}{TP}{True positive}

\newacronym {WP}{WP}{Wind park}
\newacronym {wrt}{w.r.t.}{With respect to}
 

